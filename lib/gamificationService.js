...
export const BADGES = {
  FOODIE_ADVENTURER: {
    ...
    iconUrl: '/badges/foodie.png', // Placeholder icon
 ...
 */
export const getGamificationData = async (uid) => {
  if (!uid) {
    throw new Error('No UID provided to getGamificationData');
  }
  const gamificationRef = getGamificationDocRef(uid);
...
export const addQP = async (uid, qpEvent, metadata = {}) => {
  ...
  try {
    ...
    if (qpAmount < 0) {
      const gamificationData = await getGamificationData(uid);
    ...
    const updatedGamificationData = await getGamificationData(uid);
