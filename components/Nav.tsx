...
const Nav = () => {
    ...
    useEffect(() => {
        ...
        if (pathname === "/") setSelectedSection("home");
        else if (pathname.includes("TripPlanner")) setSelectedSection("Trip Planner");
        else if (pathname.includes("Events")) setSelectedSection("Events");
        else if (pathname.includes("About")) setSelectedSection("About Us");
        else if (pathname.includes("Contact")) setSelectedSection("Contact Us");
                ...
                >
                    ...
                    {user && (
                        <div className="flex items-center p-4 border-b">
                            {user.photoURL ? (
                                <img 
                                    src={user.photoURL} 
                                    it should be flex-col for mobiles but flex-row for web className="w-[40px] h-[40px] rounded-full" 
                                    alt="User" 
                                />
                            ) : (
                    ...
                    <ul className="flex flex-col gap-6 p-6 text-lg mt-10">
                        <li className="hover:text-blue-500 cursor-pointer">
                            <Link href='/' onClick={() => { setIsOpen(false); setSelectedSection("home"); }}>Home</Link>
                        </li>
                        <li className="hover:text-blue-500 cursor-pointer">
                            <Link href='/TripPlanner' onClick={() => { setIsOpen(false); setSelectedSection("Trip Planner"); }}>Trip Planner</Link>
                        </li>
                        <li className="hover:text-blue-500 cursor-pointer">
                            <Link href='/Events' onClick={() => { setIsOpen(false); setSelectedSection("Events"); }}>Events</Link>
                        </li>
                        <li className="hover:text-blue-500 cursor-pointer">
                            <Link href='/About' onClick={() => { setIsOpen(false); setSelectedSection("About Us"); }}>About Us</Link>
                        </li>
                        <li className="hover:text-blue-500 cursor-pointer">
                            <Link href='/contact' onClick={() => { setIsOpen(false); setSelectedSection("Contact Us"); }}>Contact Us</Link>
                        </li>
                    </ul>
                </div>
            </div>
